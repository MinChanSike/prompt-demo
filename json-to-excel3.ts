...
/**
 * Type definitions for the JSON to Excel conversion system
 */

// Utils interface for better TypeScript support
interface UtilsInterface {
  readJsonFile(filename: string): any;
  writeJsonFile(filename: string, data: any): void;
  writeExcelFile(filename: string, data: any[][], worksheetName: string): void;
}

// Base types for incident data structure
...
interface Formatters {
  [fieldPath: string]: FormatterFunction;
}
 ...
 */
function timeFormatter(value: any): string {
 ...
 * @param value - Plate number serial
 * @returns Uppercase plate number serial
 */
function plateNumberFormatter(value: any): string {
  return value ? value.toString().toUpperCase() : value;
}

/**
 * Formats personnel names to proper case
 * @param value - Personnel name
 * @returns Properly formatted name
 */
function nameFormatter(value: any): string {
  if (!value) return value;
  return value
    .toString()
    .toLowerCase()
 ...
 * @param value - Number of casualties
 * @returns Formatted casualties description
 */
function casualtiesFormatter(value: any): string {
 ...
 * @param fieldPath - Dot-notation path (e.g., "location.road")
 * @returns The value at the specified path, or undefined if not found
 */
function getValueByPath(obj: any, fieldPath: string): any {
 ...
 * @param formatters - The formatters configuration object
...
function applyFormatter(
  ...
  try {
    ...
    console.warn(
      `Formatter error for field '${field}':`,
 ...
 * @param contextRules - Rules defining how to identify unique contexts
 * @param incidentId - Current incident ID for scoping
 * @returns Unique context key for deduplication
 */
 ...
 * @param config - Configuration object containing fields and rules
 * @returns Array of flattened row objects
 */
function flattenAndDedup(obj: any, config: Config): Row[] {
  const { fields, parentKey, contextRules, formatters } = config;
  let seen: { [key: string]: Set<any> } = {}; // Tracks seen values per context to enable deduplication
 ...
 *
 * @param rows - Array of row objects to potentially merge
 * @param config - Configuration object
 * @returns Array of merged row objects
 */
function mergeRowsByParent(rows: Row[], config: Config): Row[] {
  const { fields, parentKey, noParentKey } = config;

  // Group rows by their parent key (incident ID)
  const grouped: GroupedRows = rows.reduce((acc: GroupedRows, row: Row) => {
    const key = (row[parentKey] as string) || noParentKey;
    (acc[key] = acc[key] || []).push(row);
    return acc;
  }, {});

  // Process each group to merge compatible rows
  return Object.values(grouped).flatMap((groupRows: Row[]) => {
    ...
    for (let i = 0; i < groupRows.length; i++) {
      ...
      for (let j = i + 1; j < groupRows.length; j++) {
        ...
        if (fields.every((f: string) => !row[f] || !groupRows[j][f])) {
 ...
 * @param allRows - Array of processed row objects
 * @param config - Configuration object
 * @returns Array of row objects with blank separators
 */
function addBlankRowsBetweenIncidents(allRows: Row[], config: Config): Row[] {
  ...
  for (let i = 0; i < allRows.length; i++) {
    const currentIncidentId = allRows[i][parentKey] as string;
 ...
 * @returns The processed data as a 2D array
 */
function processData(config: Config = CONFIG): (string | number)[][] {
  try {
    console.log(`Reading input file: ${config.inputFile}`);
    const inputData: Incident[] = (Utils as UtilsInterface).readJsonFile(
      config.inputFile
    );

    console.log("Processing data...");
    // Process each incident through flattening and merging
    let allRows = inputData.flatMap((item: Incident) =>
    ...
    (Utils as UtilsInterface).writeJsonFile(config.outputJsonFile, rows);

    console.log(`⌛ Writing Excel output: ${config.outputExcelFile}`);
    (Utils as UtilsInterface).writeExcelFile(
      config.outputExcelFile,
      rows,
